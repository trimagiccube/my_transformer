...
**代码级确认**(forward 里 FFN 三步,形状一目了然):
```c
int hidden_dim = p->hidden_dim;                       // 768
matmul(s->hb,  s->xb, w->w1, dim, hidden_dim);        // ⑧ 升维 288 → 768
matmul(s->hb2, s->xb, w->w3, dim, hidden_dim);        //    升维 288 → 768(第二路)
f_silu_elementwise_mul_w3(s, hidden_dim);             // ⑨ SwiGLU 在 768 维上算
matmul(s->xb,  s->hb, w->w2, hidden_dim, dim);        // ⑩ 降维 768 → 288
```
- 缓冲区 `s->hb` / `s->hb2` 在 `malloc_run_state` 里按 `hidden_dim` 分配 → 768 大小;`s->x` 按 `dim` → 288。
- 膨胀**只在 FFN 内部**:注意力全程 288,出了 FFN 立刻压回 288,主干维度恒定。
